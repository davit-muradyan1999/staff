<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.citizenships.index')">
      ...
      <form @submit.prevent="submitForm" @keyup.13="submitForm">
        <div class="mb-3"  v-for="(lang, index) in $page.props.languages" :key="lang.id">
          ...
          <input
            type="text"
            class="form-control"
            v-model="citizenship.name[lang.code]"
            :id="`name-${lang.code}`"
...
const v$ = useVuelidate(rules, citizenship)

const submitForm = () => {
  v$.value.$touch()
  if (v$._value.$invalid) return false

  route().current('admin.citizenships.create')
    ? citizenship.post(route('admin.citizenships.store'), citizenship)
    : citizenship.put(route('admin.citizenships.update', citizenship.id), citizenship)
