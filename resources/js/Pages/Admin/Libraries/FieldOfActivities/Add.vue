<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.field_of_activities.index')">
      ...
      <form @submit.prevent="submitForm" @keyup.13="submitForm">
        <div class="mb-3"  v-for="(lang, index) in $page.props.languages" :key="lang.id">
          ...
          <input
            type="text"
            class="form-control"
            v-model="fieldOfActivity.name[lang.code]"
            :id="`name-${lang.code}`"
...
const v$ = useVuelidate(rules, fieldOfActivity)

const submitForm = () => {
  v$.value.$touch()
  if (v$._value.$invalid) return false

  route().current('admin.field_of_activities.create')
    ? fieldOfActivity.post(route('admin.field_of_activities.store'), fieldOfActivity)
    : fieldOfActivity.put(route('admin.field_of_activities.update', fieldOfActivity.id), fieldOfActivity)
