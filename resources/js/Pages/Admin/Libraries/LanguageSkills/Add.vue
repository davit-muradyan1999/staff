<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.language_skills.index')">
      ...
      <form @submit.prevent="submitForm" @keyup.13="submitForm">
        <div class="mb-3"  v-for="(lang, index) in $page.props.languages" :key="lang.id">
          ...
          <input
            type="text"
            class="form-control"
            v-model="languageSkill.name[lang.code]"
            :id="`name-${lang.code}`"
...
const v$ = useVuelidate(rules, languageSkill)

const submitForm = () => {
  v$.value.$touch()
  if (v$._value.$invalid) return false

  route().current('admin.language_skills.create')
    ? languageSkill.post(route('admin.language_skills.store'), languageSkill)
    : languageSkill.put(route('admin.language_skills.update', languageSkill.id), languageSkill)
