    ...
    <div class="tab-content">
        ...
        <div class="card mb-4">
          <div class="card-body">
            <div class="row bg-white">
              <div class="col-md-6">{{ $t('general.summation') }}<span class="h4 ms-1 text-danger">{{ useFixedNumber(totalWaitingJobsSpending) }}</span>
            ...
            <div class="card-body p-4 position-relative testimonial-group">
              <div class="row bg-white">
                ...
                <div class="col border-end">
                 ...
                 <div class="col">
                  <span class="float-start me-1">{{ useFixedNumber(job?.spending_waiting_total) }} {{ $t('general.rub') }}</span> ({{ $t('general.spending') }})
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="tab-pane" :class="{'active': activeTab == 'history', 'show': activeTab == 'history'}" id="pills-history" role="tabpanel" aria-labelledby="pills-history-tab">
        <div class="card mb-4">
          <div class="card-body">
            <div class="row bg-white">
              <div class="col-md-6">{{ $t('general.summation') }}<span class="h4 ms-1 text-danger">{{ useFixedNumber(totalHistoryJobsSpending) }}</span>
                <span class="ms-1 text-danger">руб</span>
              </div>
            </div>
          </div>
              ...
              <div class="row bg-white">
                ...
                <div class="col border-end">
                  <span class="float-start" :class="{
                    ...
                    'text-decoration-line-through': job?.jcl_status != 'CONFIRMED'
                  }">{{ useFixedNumber(job?.spending_history_total) }} {{ $t('general.rub') }} ({{ $t('general.spending') }})</span>
                </div>
                <div class="col border-end">
...
const props = defineProps({
  ...
  startedAt: String,
  finishedAt: String,
  totalWaitingJobsSpending: Number,
  totalHistoryJobsSpending: Number,
});
...
watch(search.value, (currentValue, oldValue) => {
  router.get(route('admin.spending'), {
    search: true,
