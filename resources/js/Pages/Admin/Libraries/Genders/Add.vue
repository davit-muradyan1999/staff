<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.genders.index')">
...
const v$ = useVuelidate(rules, gender)

const submitForm = () => {
  v$.value.$touch()
  if (v$._value.$invalid) return false

  route().current('admin.genders.create')
    ? gender.post(route('admin.genders.store'), gender)
    : gender.put(route('admin.genders.update', gender.id), gender)
