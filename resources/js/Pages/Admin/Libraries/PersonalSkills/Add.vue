<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.personal_skills.index')">
      ...
      <form @submit.prevent="submitForm" @keyup.13="submitForm">
        <div class="mb-3"  v-for="(lang, index) in $page.props.languages" :key="lang.id">
          ...
          <input
            type="text"
            class="form-control"
            v-model="personalSkill.name[lang.code]"
            :id="`name-${lang.code}`"
...
const v$ = useVuelidate(rules, personalSkill)

const submitForm = () => {
  v$.value.$touch()
  if (v$._value.$invalid) return false

  route().current('admin.personal_skills.create')
    ? personalSkill.post(route('admin.personal_skills.store'), personalSkill)
    : personalSkill.put(route('admin.personal_skills.update', personalSkill.id), personalSkill)
