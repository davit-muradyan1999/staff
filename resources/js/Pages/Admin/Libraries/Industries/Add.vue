<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.industries.index')">
      ...
      <form @submit.prevent="submitForm" @keyup.13="submitForm">
        <div class="mb-3"  v-for="(lang, index) in $page.props.languages" :key="lang.id">
          ...
          <input
            type="text"
            class="form-control"
            v-model="industry.name[lang.code]"
            :id="`name-${lang.code}`"
...
const submitForm = () => {
  ...
  route().current('admin.industries.create')
    ? industry.post(route('admin.industries.store'), industry)
    : industry.put(route('admin.industries.update', industry.id), industry)
