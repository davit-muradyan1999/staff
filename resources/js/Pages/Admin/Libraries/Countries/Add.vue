<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.countries.index')">
      ...
      <form @submit.prevent="submitForm" @keyup.13="submitForm">
        <div class="mb-3"  v-for="(lang, index) in $page.props.languages" :key="lang.id">
          ...
          <input
            type="text"
            class="form-control"
            v-model="country.name[lang.code]"
            :id="`name-${lang.code}`"
...
const submitForm = () => {
  ...
  route().current('admin.countries.create')
    ? country.post(route('admin.countries.store'), country)
    : country.put(route('admin.countries.update', country.id), country)
