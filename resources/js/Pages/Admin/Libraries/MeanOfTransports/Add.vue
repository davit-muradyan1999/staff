<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.mean_of_transports.index')">
      ...
      <form @submit.prevent="submitForm" @keyup.13="submitForm">
        <div class="mb-3"  v-for="(lang, index) in $page.props.languages" :key="lang.id">
          ...
          <input
            type="text"
            class="form-control"
            v-model="meanOfTransport.name[lang.code]"
            :id="`name-${lang.code}`"
...
const v$ = useVuelidate(rules, meanOfTransport)

const submitForm = () => {
  v$.value.$touch()
  if (v$._value.$invalid) return false

  route().current('admin.mean_of_transports.create')
    ? meanOfTransport.post(route('admin.mean_of_transports.store'), meanOfTransport)
    : meanOfTransport.put(route('admin.mean_of_transports.update', meanOfTransport.id), meanOfTransport)
