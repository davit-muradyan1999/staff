<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.disability_groups.index')">
      ...
      <form @submit.prevent="submitForm" @keyup.13="submitForm">
        <div class="mb-3"  v-for="(lang, index) in $page.props.languages" :key="lang.id">
          ...
          <input
            type="text"
            class="form-control"
            v-model="disabilityGroup.name[lang.code]"
            :id="`name-${lang.code}`"
...
const v$ = useVuelidate(rules, disabilityGroup)

const submitForm = () => {
  v$.value.$touch()
  if (v$._value.$invalid) return false

  route().current('admin.disability_groups.create')
    ? disabilityGroup.post(route('admin.disability_groups.store'), disabilityGroup)
    : disabilityGroup.put(route('admin.disability_groups.update', disabilityGroup.id), disabilityGroup)
