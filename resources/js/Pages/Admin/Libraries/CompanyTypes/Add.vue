<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.company_types.index')">
      ...
      <form @submit.prevent="submitForm" @keyup.13="submitForm">
        <div class="mb-3"  v-for="(lang, index) in $page.props.languages" :key="lang.id">
          ...
          <input
            type="text"
            class="form-control"
            v-model="companyType.name[lang.code]"
            :id="`name-${lang.code}`"
...
const v$ = useVuelidate(rules, companyType)

const submitForm = () => {
  v$.value.$touch()
  if (v$._value.$invalid) return false

  route().current('admin.company_types.create')
    ? companyType.post(route('admin.company_types.store'), companyType)
    : companyType.put(route('admin.company_types.update', companyType.id), companyType)
