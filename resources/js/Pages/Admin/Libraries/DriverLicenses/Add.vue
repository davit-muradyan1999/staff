<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.driver_licenses.index')">
      ...
      <form @submit.prevent="submitForm" @keyup.13="submitForm">
        <div class="mb-3"  v-for="(lang, index) in $page.props.languages" :key="lang.id">
          ...
          <input
            type="text"
            class="form-control"
            v-model="driverLicense.name[lang.code]"
            :id="`name-${lang.code}`"
...
const v$ = useVuelidate(rules, driverLicense)

const submitForm = () => {
  v$.value.$touch()
  if (v$._value.$invalid) return false

  route().current('admin.driver_licenses.create')
    ? driverLicense.post(route('admin.driver_licenses.store'), driverLicense)
    : driverLicense.put(route('admin.driver_licenses.update', driverLicense.id), driverLicense)
