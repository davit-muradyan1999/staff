<template>
  ...
  <div class="card mb-4 card-table-list">
    <div class="card-header">
      ...
      {{ title }}
      <Link class="edit-action-link" :href="route('admin.quantity_employees.index')">
      ...
      <form @submit.prevent="submitForm" @keyup.13="submitForm">
        <div class="mb-3"  v-for="(lang, index) in $page.props.languages" :key="lang.id">
          ...
          <input
            type="text"
            class="form-control"
            v-model="quantityEmploye.name[lang.code]"
            :id="`name-${lang.code}`"
...
const v$ = useVuelidate(rules, quantityEmploye)

const submitForm = () => {
  v$.value.$touch()
  if (v$._value.$invalid) return false

  route().current('admin.quantity_employees.create')
    ? quantityEmploye.post(route('admin.quantity_employees.store'), quantityEmploye)
    : quantityEmploye.put(route('admin.quantity_employees.update', quantityEmploye.id), quantityEmploye)
