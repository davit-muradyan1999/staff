<template>
  ...
  <div class="mt-3 text-end">
    <Link v-if="usePermissionM([3,4])" class="edit-action-link d-inline float-end add-job" :href="$page.props.auth?.user?.role == 'COMPANY' && !$page.props.auth?.user?.company_name ? '#' : route('company_jobs_add') + searchURL">
      <button :disabled="$page.props.auth?.user?.role == 'COMPANY' && !$page.props.auth?.user?.company_name" class="btn btn-success ps-3 pe-3 pt-2 pb-2 me-3" type="button">
        {{ $t('general.addJob') }}
      </button>
    </Link>
  </div>
  ...
  <div class="row mt-3" style="position: relative; z-index: 9999;">
    ...
    <div class="col-md-4 mb-2" v-if="usePermissionM([1,3,4])">
      <Multiselect
